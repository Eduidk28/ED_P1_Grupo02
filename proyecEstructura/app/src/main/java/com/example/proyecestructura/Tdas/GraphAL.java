...
public class GraphAL<V, E> implements Serializable {
    ...
    public boolean disconnect(V content1, V content2) {
        ...
        // Eliminar arista de v1 a v2
        for (int i = 0; i < v1.getEdges().size(); i++) {
            if (v1.getEdges().get(i).getTarget().equals(v2)) {
        ...
        if (!isDirected) {
            for (int i = 0; i < v2.getEdges().size(); i++) {
                if (v2.getEdges().get(i).getTarget().equals(v1)) {
    ...
    public boolean removeEdge(V from, V to) {
        Vertex<V, E> v1 = getVertexByContent(from);
        Vertex<V, E> v2 = getVertexByContent(to);

        if (v1 == null || v2 == null) return false;

        boolean removed = v1.getEdges().removeIf(edge -> edge.getTarget() == v2);

        if (!isDirected) {
            v2.getEdges().removeIf(edge -> edge.getTarget() == v1);
        }
