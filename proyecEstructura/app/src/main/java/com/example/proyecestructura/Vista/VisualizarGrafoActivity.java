        ...
        eliminarVueloLauncher = registerForActivityResult(
                ...
                result -> {
                    if (result.getResultCode() == RESULT_OK && mMap != null) {
                        mMap.clear();
                        onMapReady(mMap);  // Redibuja aeropuertos y vuelos
                    }
                }
        );
        ...
        detalleAeropuertoLauncher = registerForActivityResult(
                ...
                result -> {
                    if (result.getResultCode() == RESULT_OK && mMap != null) {
                        ...
                        onMapReady(mMap);   // Redibujar aeropuertos y vuelos
                    }
                }
        );
...
// Agrupar aristas por origen->destino
...
// Dibujar aristas separadas
        ...
        // 2️⃣ Dibujar círculos negros de aeropuertos encima de las aristas
        for (Vertex<Aeropuerto, Vuelo> v : grafo.getVertices()) {
    ...
    // Contar aristas entre el mismo par de aeropuertos
    private int contarAristasEntre(Vertex<Aeropuerto, Vuelo> origen, Vertex<Aeropuerto, Vuelo> destino) {
        int count = 0;
        for (Edge<Vuelo, Aeropuerto> e : origen.getEdges()) {
            if (e.getTarget().equals(destino)) count++;
        }
        return count;
    }

    private int obtenerIndiceAristaEntre(Vertex<Aeropuerto, Vuelo> origen, Vertex<Aeropuerto, Vuelo> destino, Edge<Vuelo, Aeropuerto> arista) {
        int index = 0;
        for (Edge<Vuelo, Aeropuerto> e : origen.getEdges()) {
            if (e.getTarget().equals(destino)) {
                if (e.equals(arista)) return index;
                index++;
            }
        }
        return index;
    ...
    // Dibujar arista con separación y peso sobre línea
    private void drawEdgeWithWeightAndArrow(LatLng start, LatLng end, int pesoKm, int index, int totalAristas) {
        ...
        double offset = 0.05; // ajustar según zoom
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        mMap.setOnMarkerClickListener(marker -> {
            ...
            intent.putStringArrayListExtra("aerolineas", aerolineas); // 👈 aquí estaba el faltante
