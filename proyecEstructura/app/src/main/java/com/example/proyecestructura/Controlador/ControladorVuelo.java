...
public class ControladorVuelo {
    ...
    public boolean eliminarVuelo(Aeropuerto origen, Aeropuerto destino, String aerolinea, Context context) {
        Vertex<Aeropuerto, Vuelo> v = grafo.getVertexByContent(origen);
        if (v != null) {
            for (int i = 0; i < v.getEdges().size(); i++) {
                Edge<Vuelo, Aeropuerto> e = v.getEdges().get(i);
                if (e.getTarget().getContent().equals(destino) && e.getData().getAerolinea().equals(aerolinea)) {
                    v.getEdges().remove(i);
                    PersistenciaGrafo.guardar(context);
                    return true;
                }
